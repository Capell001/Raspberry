...
class App:
    def __init__(self,window):
        #init firebase
        self.cred = credentials.Certificate('/home/pi/Documents/iot-aloni-firebase-adminsdk-5ufsz-eb84d47e9f.json')
        firebase_admin.initialize_app(self.cred, {
    'databaseURL': 'https://iot-aloni.firebaseio.com/',
    'databaseAuthVariableOverride':{'uid': 'SS8yq23eReZ4cD2Z4Mr0RpXFgZp1'}
    })
    ...
    def rfidHandler(self):
        ...
        SG90Ref = db.reference(path='iot0624/SG90')
        SG90Status = SG90Ref.get()
        LEDRef = db.reference(path='iot0624/LED')
        LEDStatus = LEDRef.get()
                ...
                if uid0 != self.preUid0 or uid1 != self.preUid1 or uid2 != self.preUid2 or uid3 != self.preUid3:
                    ...
                    try:
                        ...
                        if info != None:
                            ...
                            #Lcd String
                            WelString = "Welcome!" +" "+info["name"]
                            self.firestore.collection('Door').document().set(member)
                            self.lcd.display_string(WelString,1);
                            self.lcd.display_string(date,2);
